...
function renderProxyHistoryList() {
  ...
  for (const row of appState.proxyHistoryRows) {
    ...
    item.className = 'scroll-item proxy-history-list-item';
    if (row.id === appState.selectedProxyHistoryId) {
      item.classList.add('active');
    }

    item.innerHTML = `
      <div class="proxy-history-primary"><strong>${req.method}</strong> ${req.uri}</div>
      <small class="proxy-history-inline-meta">${req.host} · ${resp ? resp.status : 'no response'} · ${row.exchange.duration_ms}ms</small>
    `;
